...
class Test_XML_Parser(unittest.TestCase):
    def test_parse_xml(self):
        ...
        self.assertDictEqual(
            test_xml, {"0123456789": {"ethnicity": "African", "image": "African1"}}
        )

    def test_get_imgpath_from_uid(self):
        test_img = XML_Parser.get_imgpath_from_uid(
            None, "newganmanager/test/test.xml", "0123456789"
        )
        self.assertEqual(test_img, "African/African1")

    def test_get_imgpath_from_uid_negative(self):
        test_img = XML_Parser.get_imgpath_from_uid(
            None, "newganmanager/test/test.xml", "0000000000"
        )
...
class Test_RTF_Parser(unittest.TestCase):
    ...
    def test_parse_rtf_with_short_UIDs(self):
        test_simple_UID = RTF_Parser.parse_rtf(
            None, "newganmanager/test/test_simple_UID.rtf"
        )
    ...
    def test_parse_rtf_fake_players(self):
        test_simple_UID_fake = RTF_Parser.parse_rtf(
            None, "newganmanager/test/test_simple_UID_fake.rtf"
        )
        self.assertSequenceEqual(
            test_simple_UID_fake[0], ["191571", "ESP", "BAS", "1"]
        )
        self.assertSequenceEqual(
            test_simple_UID_fake[1], ["1915576430", "KSA", "ARG", "2"]
        )
        self.assertSequenceEqual(
            test_simple_UID_fake[2], ["1915576", "KSA", "GER", "2"]
        )
        self.assertEqual(len(test_simple_UID_fake), 3)

    def test_valid_rtf(self):
        self.assertTrue(
            RTF_Parser.is_rtf_valid(None, "newganmanager/test/test_simple_UID.rtf")
        )
...
class Test_Config_Manager(unittest.TestCase):
    def test_get_latest_prf(self):
        latest_prf = Config_Manager().get_latest_prf(
            "newganmanager/test/simple_cfg.json"
        )
...
class Test_Xml_Writing(unittest.TestCase):
    def setUp(self):
        ...
        self.xml_data = self.pm.write_xml(self.data, True)
    ...
    def test_write_xml_players_mapped_in_file(self):
        with open(
            self.pm.prf_cfg["img_dir"] + "config.xml", "r", encoding="UTF-8"
        ) as fp:
            xml_file = fp.read()
...
class Test_Profile_Manager(unittest.TestCase):
    ...
    def test_delete_profile(self):
        ...
        f.close()
        self.pm.config = Config_Manager().load_config(
            "newganmanager/testing/.user/cfg.json"
        )
    ...
    def test_swap_xml(self):
        self.pm.swap_xml(
            "test", "No Profile", "newganmanager/test/", "newganmanager/test/"
        )
    ...
    def test_switching_profiles_with_invalid_path(self):
        self.pm.swap_xml(
            "test", "No Profile", "newganmanager/invalid/", "newganmanager/test/"
        )
        self.pm.swap_xml(
            "No Profile", "test", "newganmanager/test/", "newganmanager/invalid/"
        )
