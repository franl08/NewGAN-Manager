...
class Profile_Manager(config_manager.Config_Manager):
    ...
    def write_xml(self, data, append_r_prefix):
        with open(
            self.root_dir + "/.config/config_template", "r", encoding="UTF-8"
        ) as fp:
    ...
    def swap_xml(self, deact_name, act_name, deact_img_dir, act_img_dir):
        if os.path.isfile(deact_img_dir + "config.xml"):
            with open(
                self.root_dir + "/.user/" + deact_name + ".xml", "wb"
            ) as output, open(deact_img_dir + "config.xml", "rb") as input:
                copyfileobj(input, output)

        if os.path.isfile(act_img_dir + "config.xml"):
            with open(act_img_dir + "config.xml", "wb") as output, open(
                self.root_dir + "/.user/" + act_name + ".xml", "rb"
            ) as input:
                copyfileobj(input, output)
